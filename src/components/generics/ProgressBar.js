...
const Steps = ({ currentStep }) => (
    <div className="flex justify-center items-center flex-wrap max-w-4xl w-full px-4 sm:px-6">
        {steps.map((step, index) => {
            ...
            const textStyle = isActive || isCompleted ? 'font-bold text-gray-900' : 'text-gray-500';
            const circleBg = isActive ? 'bg-[#d3986a] text-white' : 'bg-transparent text-gray-500 border border-gray-400';

            return (
                <React.Fragment key={step}>
                    <div className={`flex items-center whitespace-nowrap text-sm ${textStyle} mx-1`}>
                        {/* Step Number Circle */}
                        <span className={`w-[18px] h-[18px] rounded-full flex justify-center items-center mr-2 text-[10px] font-bold ${circleBg}`}>
                            {stepNumber}
                        </span>
                        {step}
                    </div>
