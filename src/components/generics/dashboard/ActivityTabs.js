          ...
          {activeTab === "bookings" ? (
            bookings.length > 0 ? (
              bookings.map((booking) => (
                <div
                  key={booking.id}
                  className="flex items-center hover:bg-gray-50 p-2 rounded-md transition-colors cursor-pointer group"
                  onClick={() =>
                    setActiveChat({ id: booking.id, name: booking.client_name })
                  }
                >
                  <div className="flex-1">
                    <p className="text-gray-700 font-medium group-hover:text-theme-primary transition-colors">
                      {booking.client_name}
            ...
            messages.map((msg) => (
              <div
                ...
                className="flex items-center hover:bg-gray-50 p-2 rounded-md transition-colors cursor-pointer group"
                onClick={() =>
                  setActiveChat({ id: msg.id, name: msg.other_party_name })
                }
              >
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-800 group-hover:text-theme-primary transition-colors">
